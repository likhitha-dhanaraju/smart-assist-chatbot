...
def download_images(folder, filename, product_data):

    try:
        image_urls = [value for key, value in product_data.items() if 'img_url' in key]
    except:
        image_urls = product_data['img_url']

    for i, image_url in enumerate(image_urls):
        res = requests.get(image_url, stream=True)
        if res.status_code == 200:
            if not os.path.exists(os.path.join(folder, filename + '_' + str(i) + '.jpg')):
                with open(os.path.join(folder, filename + '_' + str(i) + '.jpg'), 'wb') as f:
...
def amazon_batch_scrapping(url_folder):
    product_folder = "04 Product Data"

    if not os.path.exists(product_folder):
        os.mkdir(product_folder)

    url_files = [i for i in os.listdir(url_folder) if '.csv' in i]

    for url_filename in tqdm(url_files):

        try:

            category_folder = url_filename[6:].split('.csv')[0].replace("!@#$%^&*()[]{};:,./<>?\|`~-=_+", "")
            print("\nExtracting data for :", category_folder)

            if not os.path.exists(os.path.join(product_folder, category_folder)):
                os.mkdir(os.path.join(product_folder, category_folder))

            data = pd.read_csv(os.path.join(url_folder, url_filename))

            url_list = data['urls']

            # if not len(url_list) == len(os.listdir(os.path.join(product_folder, category_folder))):

            for url in tqdm(url_list):

                product_name = url.strip().split("dp/")[-1]
                if len(product_name) != 10 and url != "https://www.amazon.com/errors/500":
                    product_name = url.strip().split("dp%2F")[-1][:10]

                product_data_folder = "Product_" + product_name

                # product_data_folder = ''.join(e for e in product_data_folder if e.isalnum())
                save_folder = os.path.join(product_folder, category_folder, product_data_folder)

                if url != "https://www.amazon.com/errors/500":

                    if not os.path.exists(save_folder):
                        os.mkdir(save_folder)

                    if not os.path.exists(os.path.join(save_folder, product_data_folder + '.json')):

                        product_data = main(url)
                        with open(os.path.join(save_folder, product_data_folder + '.json'), 'w') as f:
                            json.dump(product_data, f)

                        download_images(folder=save_folder, filename=product_data_folder, product_data=product_data)
                    ...
                    if len(os.listdir(save_folder)) == 1:
                        ...
                        try:
                            ...
                            for i, image_url in enumerate(total_image_urls):
                                res = requests.get(image_url, stream=True)
                                if res.status_code == 200:
                                    if not os.path.exists(os.path.join(save_folder, product_data_folder + '_' + str(i) + '.jpg')):
                                        with open(os.path.join(save_folder, product_data_folder + '_' + str(i) + '.jpg'), 'wb') as f:
                                            shutil.copyfileobj(res.raw, f)
                                        # print('Image sucessfully Downloaded')
                                else:
                                    print('Image Couldn\'t be retrieved')

                            with open(os.path.join(save_folder, product_data_folder + '.json'),  encoding='utf-8') as f:
                                product_data = json.load(f)

                            product_data.update(total_image_urls_dict)

                            with open(os.path.join(save_folder, product_data_folder + '.json'), "w") as fp:
